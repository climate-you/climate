...
def max_dry_spell_summer_per_year(
    da: xr.DataArray,
    *,
    dry_day_threshold_mm: float = 1.0,
    north_summer_months: tuple[int, ...] = (6, 7, 8),
    south_summer_months: tuple[int, ...] = (12, 1, 2),
) -> xr.DataArray:
    tname = find_time_dim(da)
    if not np.issubdtype(da[tname].dtype, np.datetime64):
        da = xr.decode_cf(da.to_dataset(name="v"))["v"]
    ...
    months_all = da_t[tname].dt.month.values.astype(int)
    ...
    north_months = np.asarray(tuple(int(m) for m in north_summer_months), dtype=int)
    south_months = np.asarray(tuple(int(m) for m in south_summer_months), dtype=int)
    is_north = (lat_vals >= 0.0)[None, :, None]
    ...
    for year in years:
        ...
        yvals = vals[idx_year, :, :]
        ymonths = months_all[idx_year]
        north_mask = np.isin(ymonths, north_months)[:, None, None]
        south_mask = np.isin(ymonths, south_months)[:, None, None]
        summer_mask = np.where(is_north, north_mask, south_mask)

        obs = np.isfinite(yvals)
        dry = (yvals < dry_threshold) & obs & summer_mask
    ...
    if not out_vals:
        return xr.DataArray(
            ...
            name="max_dry_spell_summer_per_year",
        )
